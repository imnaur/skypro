...
# проверяем EXECUTED
def test_filter_by_state_valid(list_of_states_1: list[dict], list_of_states_2: list[dict]) -> None:
    ...
    expected = [{"id": 4, "state": "CANCELED"}, {"id": 6, "state": "CANCELED"}]
    assert result == expected


# Проверка через вывод ошибки ввода
@pytest.mark.parametrize("inp", [[], [{"id": 1, "state": ""}], [{"id": 2, "state": None}]])
def test_filter_by_state_invalid(inp: list) -> None:
    with pytest.raises(ValueError):
        filter_by_state(inp, state="")


# Проверка кода по датам в порядке убывания или возрастания
def test_sort_by_date_valid(list_of_dates: list[dict]) -> None:
    expected = [
